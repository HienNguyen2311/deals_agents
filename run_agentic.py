...
import time
import gradio as gr
from lib.deal_agent_framework import DealAgentFramework
from lib.log_utils import reformat
...
class App:
    ...
    def run(self):
        with gr.Blocks(title="HOT DEALS!!!", fill_width=True) as ui:
            ...
            with gr.Row():
                gr.Markdown('<div style="text-align: center;font-size:24px"><strong>' \
                'The Price is Right</strong> - Autonomous Agent Framework that hunts for deals</div>')
            with gr.Row():
                gr.Markdown('<div style="text-align: center;font-size:14px">' \
                'A proprietary fine-tuned LLM deployed on Modal and a RAG pipeline with a frontier model collaborate to capture great online deals.</div>')
            with gr.Row():
